...
void mtk_charger_notify_battery_health_status(int now_temp)
{
	...
	if ((battery_health_status != pre_battery_health_status) ||
		(abs(now_temp - pre_temp) >= ZTE_BAT_TEMP_DIFF)) {
		bm_err("forcely update battery health and temperature, health: %d, temp: %d, %d\n",
			battery_health_status, now_temp, pre_temp);
		battery_main.BAT_batt_temp = now_temp;
		pre_temp = now_temp;
