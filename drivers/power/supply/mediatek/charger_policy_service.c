...
struct charger_policy_info {
	...
	struct wakeup_source 		*zte_policy_wake_source;
};
							...
							ktime_t now)
{
	...
	if (policy_info->zte_policy_wake_source)
		__pm_wakeup_event(policy_info->zte_policy_wake_source, 2000);
...
static int charger_policy_probe(struct platform_device *pdev)
{
	...
	policy_info->zte_policy_wake_source = wakeup_source_register("zte_policy_wake_source");
	if (!policy_info->zte_policy_wake_source) {
		pr_err("zte_policy_wake_source register failed\n");
	}
