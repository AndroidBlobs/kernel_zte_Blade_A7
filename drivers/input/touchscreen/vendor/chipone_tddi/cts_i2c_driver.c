...
static void cts_point_report_check(struct work_struct *work)
{
	int ret = 0;
	struct chipone_ts_data *cts_data = NULL;

	cts_data = container_of(work, struct chipone_ts_data, point_report_check_work.work);
...
#endif
#ifdef CONFIG_CTS_POINT_REPORT_CHECK
	INIT_DELAYED_WORK(&cts_data->point_report_check_work, cts_point_report_check);
#endif
